...
def apply_preselection(data, channel):
    # this time get an actual mask to deal with the weights properly
    mask = ~lep_trigger_selection(data["trigE"], data["trigM"])
    mask = mask & ~lep_type_selection(data["lep_type"])
    mask = mask & ~lep_charge_selection(data["lep_charge"])
    mask = mask & ~lep_pt_selection(data["lep_pt"])
    mask = mask & ~tight_selection(data["lep_isTightID"])
    mask = mask & ~lepton_mll_selection(calc_mass(data["lep_pt"], data["lep_eta"], data["lep_phi"], data["lep_E"]))
    mask = mask & ~lepton_eta_selection(data["lep_eta"], data["lep_type"])
    mask = mask & ~met_selection(data["met_et"])
    ...
    if channel in ("VBF", "VBF_new", "1ggF", "0ggF"):
        keep_mask = remove_jets_near_leptons(
            filtered_data["jet_eta"], filtered_data["jet_phi"],
            filtered_data["lep_eta"], filtered_data["lep_phi"],
            deltaR=0.2
        )
        if not ak.all(keep_mask):
            jet_keys = ["jet_pt", "jet_eta", "jet_phi", "jet_E", "jet_MV2c10"]
            filtered_data.update({key: filtered_data[key][keep_mask] for key in jet_keys})
